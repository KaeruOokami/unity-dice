...
{
    public sealed class RuntimeBoardSettings : IBoardSettings
    {
        readonly BoardSettings template;
        readonly PlayerBoardDefinition player1;
        readonly PlayerBoardDefinition player2;
        readonly JumboDiceSettings jumboDiceSettings;

        public RuntimeBoardSettings(
            BoardSettings template,
        ...
        public int SingleWidth => template.SingleWidth;
        public int SingleHeight => template.SingleHeight;
        ...
        public bool TryValidate(out string errorMessage) {
            if (player1.SpawnSettings == null || player2.SpawnSettings == null) {
                errorMessage = "RuntimeBoardSettings: Each player requires DiceSpawnSettings.";
                return false;
            }

            if (player1.DiceCatalog == null || player2.DiceCatalog == null) {
                errorMessage = "RuntimeBoardSettings: Each player requires DiceCatalog.";
                return false;
            }

            if (player1.SpawnSettings.InitialDiceCount != player2.SpawnSettings.InitialDiceCount) {
                errorMessage =
                    "RuntimeBoardSettings: InitialDiceCount must be shared between Player1 and Player2.";
                return false;
            }
            ...
            if (InitialDicePlacementMode == VersusInitialDicePlacementMode.Mirrored) {
                if (player1.Width != player2.Width || player1.Height != player2.Height) {
                    errorMessage =
                        "RuntimeBoardSettings: Mirrored initial dice placement requires matching board sizes.";
                    return false;
                }
            }

            if (player1.AttackSettings == null || player2.AttackSettings == null) {
                errorMessage = "RuntimeBoardSettings: Each player requires PlayerAttackSettings.";
                return false;
            }
